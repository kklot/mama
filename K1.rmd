...
Remove questions `r c(21, 23, 34:35)`

```{r new_subgroup, fig.dim = c(9, 4)}
library(tidyselect)
remove_ls <- c(21, 23, 34:35) %>% paste0("k", .)
...
bind_rows(
    "Pregnancy care" = prep_e(mpc),
    "Breastfeed" = prep_e(mbf),
    "Newborn care" = prep_e(mnc),
    "Breastfeed & Newborn care" = prep_e(mbfnc),
    .id = "sub"
) %>%
    ggplot(aes(name, `50%`)) +
        geom_hline(yintercept = 0, linetype = "dotted") +
...
Use a subset of questions related to momConnect including `r c(10:13, 31, 36)`

```{r momconnect_question, fig.dim = c(9, 4)}
include_ls <- c(10:13, 31,36) %>% paste0("k", .)

n = length(remove_ls)

dm1 <- complete(dm, 1) %>%
    select(itt, age, edu, work, partner, momconnect, area, id_, remove_ls) %>%
    mutate(score = rowSums(across(starts_with("k"))), iid = id_, n = n)
